/* COPIED FROM RELEASE OF SCIP 9.0.0 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                           */
...
#endif
#define DEFAULT_SPARSITY              4 /**< maximum number of unique weights such that a knapsack is considered as
                                         *   sparse */
#define DEFAULT_MINSPARSITY           2 /**< minimum number of unique weights such that a knapsack is considered as
                                         *   sparse */
#define DEFAULT_SEPARATECLASSICCUTS FALSE /**< whether cuts of the classical knapsack conshdlr shall be separated */
#define DEFAULT_SEPARATESPARSECUTS   TRUE /**< whether cuts of the sparse knapsack conshdlr shall be separated */
#define DEFAULT_MAXNINDEPSETS  2147483647 /**< maximum number of independent sets per cover */
#define DEFAULT_MAXNCOVERS     2147483647 /**< maximum number of covers per knapsack */
#define DEFAULT_COEFUPPERBOUND       2147483647 /**< maximum value the coefficients might have to be considered in the
                                         *   separation routine */
#define DEFAULT_USESPARSEKNAPSACKS  TRUE /**< Whether sparse knapsacks shall be used */
#define DEFAULT_SCIP_SEPAFREQ           0 /**< frequency of cut separator used by default SCIP (difference between sparse and classical cuts) */
#define DEFAULT_USESPARSEGUBS       FALSE /**< whether GUBs shall be used for separating sparse covers */
#define DEFAULT_HANDLESMALLUNITWEIGHTS TRUE /**< whether sparse data shall also be used for small knapsacks with unit weights */

#define SMALL_KNAPSACK                 10 /**< number of variables up to which knapsack is regarded as small */
#define ALLOW_COMPLCOVERMETHOD       TRUE /**< whether use the complement cover method when suited */
...
#endif
   int                   sparsity;           /**< maximum sparsity of knapsack constraint
                                              *   (i.e., number of unique weights in knapsack constraint) */
   int                   minsparsity;        /**< minimum sparsity of knapsack constraint
                                              *   (i.e., number of unique weights in knapsack constraint) */
   SCIP_Bool             separateclassiccuts; /**< whether cuts of the classical knapsack conshdlr shall be separated */
   SCIP_Bool             separatesparsecuts; /**< whether cuts of the sparse knapsack conshdlr shall be separated */
   int                   maxnindepsets;      /**< maximum number of independent sets per cover */
   int                   maxncovers;         /**< maximum number of covers per knapsack */
   int                   coefupperbound;     /**< maximum value for coefficients to be considered in the
                                              *   separation routine */
   SCIP_Bool             usesparseknapsacks; /**< whether sparse knapsacks shall be used */
   int                   scipsepafreq;       /**< sepa frequency used to separate classical cuts */
   SCIP_Bool             usesparsegubs;      /**< whether GUBs shall be used when separating sparse covers */
   SCIP_Bool             handlesmallunitweights; /**< whether sparse data shall also be used for small knapsacks with unit weights */
   SCIP_Bool             allowcomplcovermethod; /**< whether we use the complement cover method when suited  */
};


/** struct for sparse knapsack data */
struct SCIP_SparseData
{
   SCIP_Longint          capacity;           /**< capacity of knapsack */
   SCIP_Longint*         uniqueweights;      /**< unique weights of variables in knapsack constraint */
   int                   nuniqueweights;     /**< number of unique weights in knapsack constraint */
   SCIP_VAR**            vars;               /**< variables in knapsack constraint (sorted by weight) */
   int                   nvars;              /**< number of variables in knapsack constraint */
   int*                  vargroupbegins;     /**< array indicating where a new weight group of variables begins */
   int*                  covers;             /**< array of covers, concatenated */
   int                   ncovers;            /**< number of covers found */
   int                   lencovers;          /**< maximum number of elements that can be stored in covers array */
   SCIP_Longint*         coverweights;       /**< array for total weights of the covers */
   int                   lencoverweights;    /**< maximum number of elements that can be stored in coverweights array */
   int*                  picoefs;            /**< array of the pi coefficients for each cover */
   int                   picoefssize;        /**< maximum number of elements that can be stored in picoefs */
   int*                  indepsets;          /**< array of possible "+1" assignment, per covers, concatenated */
   int                   nindepsets;         /**< number of independent sets found */
   int                   indepsetssize;      /**< maximum number of elements that can be stored in indepsets */
   int*                  indepsetgroupbegins;/**< array indicating where a new group of independent set begins */
   int                   nsepacuts;          /**< number of separated cuts */
   SCIP_Bool             disabledsepa;       /**< whether sparse separation has been disabled */
};
typedef struct SCIP_SparseData SCIP_SPARSEDATA;
...
struct SCIP_ConsData
{
   ...
   SCIP_Bool             checkedsparseknapsack; /**< whether knapsack has been checked to be sparse */
   SCIP_Bool             issparseknapsack;   /**< whether the knapsack is sparse */
   SCIP_SPARSEDATA*      sparsedata;         /**< data of sparse knapsack (or NULL) */
   ...
   )
{
   ...
   (*consdata)->checkedsparseknapsack = FALSE;
   (*consdata)->issparseknapsack = FALSE;
   (*consdata)->sparsedata = NULL;
   ...
   )
{
   ...
   /* possibly free sparse data */
   if( (*consdata)->sparsedata == NULL )
   {
      SCIPfreeBlockMemory(scip, consdata);
      return SCIP_OKAY;
   }
   sparsedata = (*consdata)->sparsedata;

   if( SCIPgetSubscipDepth(scip) == 0 )
   {
      SCIPinfoMessage(scip, NULL, "success: sparsity %2d capacity %lld: weights", sparsedata->nuniqueweights,
         sparsedata->capacity);
      for( w = 0; w < sparsedata->nuniqueweights; ++w )
      {
         SCIPinfoMessage(scip, NULL, " %lld (%d)", sparsedata->uniqueweights[w],
            sparsedata->vargroupbegins[w + 1] - sparsedata->vargroupbegins[w]);
      }
      SCIPinfoMessage(scip, NULL, "\n");
   }

   SCIPfreeBlockMemoryArray(scip, &sparsedata->covers, sparsedata->lencovers);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->coverweights, sparsedata->lencoverweights);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->picoefs, sparsedata->picoefssize);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->uniqueweights, sparsedata->nuniqueweights);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->vars, sparsedata->nvars);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->vargroupbegins, sparsedata->nuniqueweights + 1);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->indepsets, sparsedata->indepsetssize);
   SCIPfreeBlockMemoryArray(scip, &sparsedata->indepsetgroupbegins, sparsedata->ncovers + 1);
   SCIPfreeBlockMemory(scip, &sparsedata);
...
/** Increment an array in reverse lexicographical order, with respect to individual upperbounds.
 *
 *  In addition, keeps track of the total weight of the array with respect to weight coefficients for each entry.
 */
static
SCIP_RETCODE getNextRevLex(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  vector,             /**< array of integers to increment */
   int                   index,              /**< index from which to start the increment */
   int                   lenvector,          /**< length of the array */
   int*                  upperbounds,        /**< array of upperbounds of each entry */
   SCIP_Longint*         weights,            /**< array of weight coef for each entry */
   SCIP_Longint*         vectorweight,       /**< pointer to the total weight of the array */
   SCIP_Bool*            success             /**< pointer to track overflow */
   )
{
   assert(scip != NULL);
   assert(vector != NULL);
   assert(index >= 0);
   assert(index < lenvector);
   assert(lenvector > 0);
   assert(upperbounds != NULL);
   assert(weights != NULL);
   assert(vectorweight != NULL);
   assert(success != NULL);
   ...
   *success = TRUE;
   ...
   ++vector[index];
   *vectorweight += weights[index];

   /* if we exceed upperbound, set to zero and transfer the access to the next position */
   while( vector[index] > upperbounds[index] )
   {
      *vectorweight -= vector[index] * weights[index];
      vector[index] = 0;

      /* detect whether we have reached the end of the search space */
      if( index + 1 == lenvector )
      {
         *success = FALSE;
         return SCIP_OKAY;
      }
      /* otherwise, increment the tail of the vector in rev-lex order */
      ++index;
      ++vector[index];
      *vectorweight += weights[index];
   }
...
/** fasts forward reverse lexicographical increment on an array by filling up a designated entry
 *
 *  In addition keeps track of the total weight of the array with respect to the weight coefficient for each entry.
 */
static
SCIP_RETCODE revLexSkipEntry(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  vector,             /**< array of integers to increment */
   int                   skipindex,          /**< index of the index that we want to skip in the search */
   int                   lenvector,          /**< length of the array */
   int*                  upperbounds,        /**< array of upperbounds of each entry */
   SCIP_Longint*         weights,            /**< array of weight coef for each entry */
   SCIP_Longint*         vectorweight,       /**< pointer to the total weight of the array */
   ...
   )
{
   ...
   *success = TRUE;
   ...
   /* fill up that entry */
   *vectorweight += (upperbounds[skipindex] - vector[skipindex]) * weights[skipindex];
   vector[skipindex] = upperbounds[skipindex];
   /* and then add one */
   SCIP_CALL( getNextRevLex(scip, vector, skipindex, lenvector, upperbounds, weights, vectorweight, success) );

   return SCIP_OKAY;
}

/** from a given seed, recursively tries to find the next minimal cover
 *
 *  If the given seed is already a minimal cover, it finishes immediately.
 *  If it is not, the program will find the next one (in reverse lexicographic order).
 *  If it reaches the end of the search-space before finding a minimal cover, stops with @p terminated = TRUE and
 *  @p seed = the zero-array.
 */
static
SCIP_RETCODE checkNextSeed(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  seed,               /**< array for the seed, has to be a cover */
   int                   nuniqueweights,     /**< number of entries in the seed/weights array */
   SCIP_Longint*         uniqueweights,      /**< array of weights for each entry of the seed */
   int*                  upperbounds,        /**< array of upperbounds for each entry of the seed */
   SCIP_Longint*         seedweight,         /**< total weight of the seed */
   SCIP_Longint          capacity,           /**< capacity of the knapsack */
   SCIP_Bool*            outofoptions,       /**< indicator if we reached the end of the search-space */
   SCIP_Bool*            iscover             /**< pointer to store the verdict of the updated seed */
   )
{
   SCIP_Bool success;
   ...
   assert(scip != NULL);
   assert(seed != NULL);
   assert(nuniqueweights > 0);
   assert(uniqueweights != NULL);
   assert(upperbounds != NULL);
   assert(seedweight != NULL);
   assert(*seedweight >= 0);
   assert(capacity >= 0);
   assert(outofoptions != NULL);
   assert(iscover != NULL);
   ...
   *iscover = FALSE;
   ...
   if( *seedweight > capacity )
   {
      SCIP_Bool isminimal;
      int i = 0;

      /* find the first non-zero entry */
      while( i < nuniqueweights && seed[i] == 0 )
         ++i;
      assert(i < nuniqueweights);

      isminimal = (capacity >= *seedweight - uniqueweights[i]);

      /* if it is minimal, we terminate the search */
      if( isminimal )
      {
         *outofoptions = FALSE;
         *iscover = TRUE;
      ...
      /* otherwise, skip this seend by incrementing by one in rev-lex order from entry i */
      SCIP_CALL( revLexSkipEntry(scip, seed, i, nuniqueweights, upperbounds, uniqueweights, seedweight,
            &success) );
   }
   else
   {
      ...
      /* if we do not have a cover yet, increment */

      /* find the maximum increase to still have a feasible solution (avoid trivial cases) */
      maxincrease = upperbounds[0] - seed[0];
      if( *seedweight + maxincrease * uniqueweights[0] > capacity )
      {
         int excess;

         excess = *seedweight + maxincrease * uniqueweights[0] - capacity;
         maxincrease -= (int)SCIPceil(scip, (SCIP_Real)excess / (SCIP_Real)uniqueweights[0]);
      }
      *seedweight += maxincrease * uniqueweights[0];
      seed[0] += maxincrease;
      assert(*seedweight <= capacity);
      assert(seed[0] <= upperbounds[0]);

      /* increment */
      SCIP_CALL( getNextRevLex(scip, seed, 0, nuniqueweights, upperbounds, uniqueweights, seedweight,
            &success) );
   }

   /* if we have reached the end of search-space, signal it */
   if( !success )
      *outofoptions = TRUE;
...
/** finds next seed using the complement technique */
static
SCIP_RETCODE checkNextSeedCompl(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  seed,               /**< array for the seed */
   int                   nuniqueweights,     /**< number of entries in the seed/weights array */
   SCIP_Longint*         uniqueweights,      /**< array of weights for each entry of the seed */
   int*                  upperbounds,        /**< array of upperbounds for each entry of the seed */
   SCIP_Longint*         seedweight,         /**< total weight of the seed */
   SCIP_Longint          capacity,           /**< capacity of the inverted knapsack */
   SCIP_Bool*            outofoptions,       /**< indicator if we reached the end of the search-space */
   SCIP_Bool*            ismaximal           /**< pointer to store the verdict of the updated seed */
   )
{
   ...
   assert(capacity >= 0);
   assert(outofoptions != NULL);
   assert(ismaximal != NULL);

   *outofoptions = FALSE;

   /* if we are over the capacity, fast forward to a new seed */
   if( *seedweight >= capacity )
   {
      *ismaximal = FALSE;
      ...
      while( i < nuniqueweights && seed[i] == 0 )
         ++i;
      assert(i < nuniqueweights);
      SCIP_CALL( revLexSkipEntry(scip, seed, i, nuniqueweights, upperbounds, uniqueweights, seedweight, &success) );
   }
   else
   {
      *ismaximal = TRUE;

      /* test if it is maximal */
      for( i = 0; i < nuniqueweights; ++i )
      {
         /* if we can add one more element to the seed without exceeding capacity then it is not maximal */
         if( seed[i] < upperbounds[i] && capacity - *seedweight > uniqueweights[i] )
         {
            *ismaximal = FALSE;
            break;
         }
      ...
      {
         /* increment */
         SCIP_CALL( getNextRevLex(scip, seed, 0, nuniqueweights, upperbounds, uniqueweights, seedweight, &success) );
      }
   }
...
/** from a given seed, iteratively finds the next minimal cover */
static
SCIP_RETCODE findNextCoverSeed(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  seed,               /**< array for the seed */
   int                   nuniqueweights,     /**< number of entries in the seed/weights array */
   SCIP_Longint*         uniqueweights,      /**< array of weights for each entry of the seed */
   int*                  upperbounds,        /**< array of upperbounds for each entry of the seed */
   SCIP_Longint*         seedweight,         /**< pointer to store total weight of the seed */
   SCIP_Longint          capacity,           /**< capacity of the knapsack */
   SCIP_Bool*            outofoptions,       /**< pointer to store whether we reached the end of the search-space */
   SCIP_Bool             inverted,           /**< whether the search is using the complement method or not */
   SCIP_Longint          knapsackweight      /**< sum of all weights in the knapsack */
   )
{
   SCIP_Bool iscover;
   ...
   assert(*seedweight >= 0);
   assert(capacity >= 0);
   assert(knapsackweight > capacity);
   assert(outofoptions != NULL);
   ...
   if( !inverted )
   {
      do
      {
         SCIP_CALL( checkNextSeed(scip, seed, nuniqueweights, uniqueweights, upperbounds, seedweight, capacity,
               outofoptions, &iscover) );
      }
      while( !(*outofoptions) && !iscover );
   }
   else
   {
      int i;
      SCIP_Longint complcapacity;
      SCIP_Longint complseedweight;
      /* need to invert the seed */
      for( i = 0; i < nuniqueweights; ++i )
         seed[i] = upperbounds[i] - seed[i];

      /* and change the capacity accordingly */
      complcapacity = knapsackweight - capacity;
      complseedweight = knapsackweight - *seedweight;

      do
      {
         SCIP_CALL( checkNextSeedCompl(scip, seed, nuniqueweights, uniqueweights, upperbounds, &complseedweight, complcapacity,
               outofoptions, &iscover) );
      }
      while( !(*outofoptions) && !iscover );

      /* invert the seed again for output */
      for( i = 0; i < nuniqueweights; ++i )
         seed[i] = upperbounds[i] - seed[i];
      *seedweight = knapsackweight - complseedweight;
   }

   return SCIP_OKAY;
}

/** Computes the pi coefficients as well as the mu function from Balas and Zemel's 1978 paper on "The Facets of the
 * Knapsack Polytope From Minimal Covers".
 * The i-th entry of the pi vector has value h if i is among the elements of the corresponding set. Alternatively,
 * the i-th entry records the largest h such that the sum of h largest weights in the cover do not exceed the weight
 * at index i. Since the weights are sorted, we can keep track of these entries efficiently.
 */
static
SCIP_RETCODE determinePiCoefs(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  cover,              /**< array for the number of each weight in the cover */
   SCIP_Longint*         uniqueweights,      /**< array of each uniqueweight of the knapsack */
   int                   nuniqueweights,     /**< number of unique weights */
   int*                  picoefs,            /**< array to store the pi coefficients */
   int                   nitemsincover,      /**< sum of entries of cover */
   SCIP_Longint*         mufunction          /**< array to store the mu function. mu(k) is equal to the sum of the k
                                              *   largest elements in the cover */
   )
{
   int maxweightcnt;
   int maxweightidx;
   ...
   assert(nuniqueweights > 0);
   assert(picoefs != NULL);
   assert(nitemsincover > 0);
   assert(mufunction != NULL);

   /* find item with largest weight in cover */
   maxweightidx = nuniqueweights - 1;
   while( maxweightidx >= 0 && cover[maxweightidx] == 0 )
      --maxweightidx;
   assert(maxweightidx >= 0);

   /* determine the mu function */
   mufunction[0] = 0;
   maxweightcnt = 0;
   for( h = 1; h < nitemsincover; ++h )
   {
      mufunction[h] = mufunction[h - 1] + uniqueweights[maxweightidx];
      ++maxweightcnt;

      /* if we have used all available items of maximum weight, find the next class of items we can use */
      if( maxweightcnt == cover[maxweightidx] )
      {
         --maxweightidx;
         while( maxweightidx >= 0 && cover[maxweightidx] == 0 )
            --maxweightidx;
   ...
   assert(maxweightcnt == cover[maxweightidx] - 1);
   mufunction[nitemsincover] = mufunction[nitemsincover - 1] + uniqueweights[maxweightidx];

   /* find the pi coefficients*/
   start = 0;
   for( i = 0; i < nuniqueweights; ++i )
   {
      picoefs[i] = mufunction[start];
      for( h = start; h < nitemsincover; ++h )
      {
         if( uniqueweights[i] < mufunction[h + 1] )
         {
            picoefs[i] = h;
            start = h;
            break;
...
/** finds the ratio between each weight and its corresponding pi coefficient plus 1 */
static
SCIP_RETCODE getSlopes(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Longint*         uniqueweights,      /**< array of unique weights in the knapsack */
   int                   nuniqueweights,     /**< number of unique weights in the knapsack */
   int*                  picoefs,            /**< array of pi coefficients for a given cover */
   SCIP_Real*            slopes              /**< array to store the quotient */
   )
{
   int i;
   ...
   for( i = 0; i < nuniqueweights; ++i )
      slopes[i] = (SCIP_Real)uniqueweights[i] / (SCIP_Real)(picoefs[i] + 1);

   return SCIP_OKAY;
}

/** get permutation to sort slopes, first by slope, second by pi coefficients */
static
SCIP_RETCODE getPermForSlopes(
   SCIP*                 scip,               /**< SCIP data structure */
   int                   nslopes,            /**< number of slopes */
   int*                  perm,               /**< array to store the permutation with slopes in non-decreasing order
                                              *   (and picoefs in non increasing order for tie breaks) */
   int*                  picoefs,            /**< array of the pi coefficients of the given cover */
   SCIP_Real*            slopes              /**< array of the slopes */
   )
{
   int i;
   int j;
   ...
   /* create a copy of the pi coefficients but with inverted value, this way we can sort them in non-increasing order */
   SCIPallocBufferArray(scip, &tmp, nslopes);
   for( i = 0; i < nslopes; ++i )
      tmp[i] = - picoefs[i];

   /* reorder the permutation and the inverted picoef arrays with respect to the sorted slopes */
   SCIPsortRealIntInt(slopes, perm, tmp, nslopes);

   /* break possible ties using the pi coefficients */
   for( i = 0; i < nslopes - 1; ++i )
   {
      j = i + 1;
      while( j < nslopes && SCIPisEQ(scip, slopes[j], slopes[i]) )
         ++j;

      if( j > i + 1 )
      {
         SCIPsortIntInt(&tmp[i], &perm[i], j - i);

         /* move the counter to the end of the group */
         i = j - 1;
      }
   }
...
/** linear search
 *  Iteratively increase the value of @p amount until either independence is violated or
 *  the x-coordinate exceeds number of items in the cover.
 *  After finding the maximal value, updates the @p priorx and @p priory with it. */
static
SCIP_RETCODE targetedLinearSearchMax(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  amount,             /**< pointer to store the final value */
   int                   picoef,             /**< entry of the pi coefficient corresponding to the value searched for */
   int                   upperbound,         /**< upper limit on the value of amount */
   SCIP_Longint          dy,                 /**< delta y corresponding to the value searched for */
   int*                  priorx,             /**< pointer to the x-coordinate of the indep set up to the current index */
   SCIP_Longint*         priory,             /**< pointer to the y-coordinate of the indep set up to the current index */
   SCIP_Longint*         mufunction,         /**< array representing the mu function for the current cover */
   int                   nitemsincover,      /**< number of items in the current cover */
   SCIP_Longint          delta               /**< difference between the weight of the cover and the capacity of the
                                              *   knapsack */
   )
{
   int i;
   ...
   *amount = 0;
   ...
   {
      /* if it is still valid for larger x, we can fast forward */
      if( x >= nitemsincover )
      {
         *amount = upperbound;
         *priorx = nitemsincover;
         *priory += ((SCIP_Longint)*amount) * dy;
      ...
      /* try with amount +1 */
      x = *priorx + picoef + 1;
      softmaxx = MIN(x, nitemsincover);
      if( *priory + dy > mufunction[softmaxx] - delta )
      {
         /* Safety check: verify if the continuous version is also valid */
         assert(x - *priorx >= 1);
         quotient = (SCIP_Real)(dy) / (x - *priorx);
         /* no need to check at @p i = *priorx, so loop over everything that is between that and the
          * min of {x, nitemsincover} (possibly empty loop) */
         for( i = *priorx + 1; i <= softmaxx && i < x; ++i)
         {
            intermediatedy = (SCIP_Real)(*priory) + (i - *priorx) * quotient;
            if( SCIPisGE(scip, mufunction[i] - delta, intermediatedy) )
               return SCIP_OKAY;
         }
...
/** find the lexicographically maximal element that is an independent set with respected to a protected prefix */
static
SCIP_RETCODE getMaxLexIndepset(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  indepset,           /**< array for the independent set */
   int                   nprotected,         /**< number of entries the protected prefix has */
   int                   nuniqueweights,     /**< number of different weights in the knapsack */
   int*                  ubslacks,           /**< array for the available number of each weights out of the cover */
   SCIP_Longint*         uniqueweights,      /**< array of the unique weights, sorted */
   int*                  picoefs,            /**< array of the pi coefficients for the current cover */
   SCIP_Longint*         mufunction,         /**< array representing the mu function for the current cover */
   int                   nitemsincover,      /**< number of items in the current cover */
   SCIP_Longint          delta,              /**< difference between the weight of the cover and the capacity of the
                                              *   knapsack */
   int*                  perm                /**< array of the permutation with respect to the slopes */
   )
{
   int i;
   SCIP_Longint sety = 0; /* x-coordinate equivalent to the set */
   int setx = 0;          /* y-coordinate equivalent to the set */

   assert(scip != NULL);
   assert(indepset != NULL);
   assert(nprotected >= 0);
   assert(nuniqueweights > 0);
   assert(ubslacks != NULL);
   assert(uniqueweights != NULL);
   assert(picoefs != NULL);
   assert(mufunction != NULL);
   assert(nitemsincover > 0);
   ...
   /* start by reading info from the prefix */
   for( i = 0; i < nprotected; ++i )
   {
      sety += indepset[perm[i]] * uniqueweights[perm[i]];
      setx += indepset[perm[i]] * (picoefs[perm[i]] + 1);
   }

   /* look for lex max */
   for( i = nprotected; i < nuniqueweights; ++i )
   {
      if( ubslacks[perm[i]] > 0 )
      {
         /* fill that entry greedily */
         SCIP_CALL( targetedLinearSearchMax(scip, &indepset[perm[i]], picoefs[perm[i]], ubslacks[perm[i]],
               uniqueweights[perm[i]], &setx, &sety, mufunction, nitemsincover, delta) );
      }
      else
         indepset[perm[i]] = 0;
   }
...
/** finds the next independent set from the given set.
 *  Using the permutation with respect to the slopes, finds the right-most index (that is not @p nuniqueweights - 1)
 *  and decreases its entry by one. Then greedily finds the lex max indep set with protected prefix up to that index.
 *  if no index is available, signal it. */
static
SCIP_RETCODE findNextIndepset(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  indepset,           /**< array for the independent set */
   int                   nuniqueweights,     /**< number of different weights in the knapsack */
   SCIP_Longint*         uniqueweights,      /**< array of the unique weights in the knapasck */
   int*                  ubslacks,           /**< array of the number of weights not in the current cover */
   int*                  picoefs,            /**< array for the pi coefficients of the current cover */
   SCIP_Longint*         mufunction,         /**< array representing the mu function of the current cover */
   int                   nitemsincover,      /**< number of items in the current cover */
   SCIP_Longint          delta,              /**< difference between the weight of the cover and the capacity */
   int*                  perm,               /**< array for the permutation w.r.t. the slopes */
   SCIP_Bool*            success             /**< pointer to signal if the function cannot find another set */
   )
{
   int i;
   ...
   *success = TRUE;
   ...
   /* if there is only 1 kind of weight, then there is no second indep set */
   if( nuniqueweights < 2 )
   {
      *success = FALSE;
      return SCIP_OKAY;
   ...
   /* find the right-most (starting at the second to last) non zero index */
   i = nuniqueweights - 2;
   while( i >= 0 && indepset[perm[i]] == 0 )
      --i;
   ...
   assert(indepset[perm[i]] > 0);
   /* Decrease that entrty */
   --indepset[perm[i]];
   /* and then fill the remaining right-hand slots greedily */
   SCIP_CALL( getMaxLexIndepset(scip, indepset, i + 1, nuniqueweights, ubslacks, uniqueweights, picoefs,
         mufunction, nitemsincover, delta, perm) );

   return SCIP_OKAY;
}

/** finds the next maximal independent set.
 *  uses the function findNextIndepset but compares the output to the independent set given.
 *  Comparable independent sets will be found in succession.
 *  If they are comparable, ignore the smaller one and search for another independent set. */
static
SCIP_RETCODE findNextMaxIndepset(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  indepset,           /**< array for the independent set */
   int*                  indepsetcopy,       /**< array to copy the indepset on */
   int                   nuniqueweights,     /**< number of different weights in the knapsack */
   SCIP_Longint*         uniqueweights,      /**< array of the unique weights in the knapasck */
   int*                  ubslacks,           /**< array of the number of weights not in the current cover */
   int*                  picoefs,            /**< array for the pi coefficients of the current cover */
   SCIP_Longint*         mufunction,         /**< array representing the mu functino of the current cover */
   int                   nitemsincover,      /**< number of items in the current cover */
   SCIP_Longint          delta,              /**< difference between the weight of the cover and the capacity */
   int*                  perm,               /**< array for the permutation w.r.t. the slopes */
   SCIP_Bool*            success             /**< pointer to signal if the function cannot find another set */
...
{
   int i;
   SCIP_Bool included = TRUE;

   assert(scip != NULL);
   assert(indepset != NULL);
   assert(indepsetcopy != NULL);
   assert(nuniqueweights > 0);
   assert(uniqueweights != NULL);
   assert(ubslacks != NULL);
   assert(picoefs != NULL);
   ...
   *success = TRUE;
   ...
   /* store the current indep set to compare */
   for( i = 0; i < nuniqueweights; ++i)
      indepsetcopy[i] = indepset[i];

   /* While the indepset that we find is included in the first one, find the next one */
   do
   {
      SCIP_CALL( findNextIndepset(scip, indepset, nuniqueweights, uniqueweights, ubslacks,
            picoefs, mufunction, nitemsincover, delta, perm, success) );

      if( !(*success) )
         break;

      /* test if the new indep set is included in the previous */
      for( i = 0; i < nuniqueweights; ++i )
      {
         if( indepset[i] > indepsetcopy[i] )
         {
            included = FALSE;
            break;
         }
...
/** change dynamically the size of an array */
static
SCIP_RETCODE increaseArraysize(
   SCIP*                 scip,               /**< SCIP data structure */
   int**                 array,              /**< pointer to the array to grow */
   int*                  arraysize,          /**< pointer to the current size of the array */
   int                   requiredsize        /**< minimum required size of the grown array */
   )
{
   int newsize;
   ...
   newsize = SCIPcalcMemGrowSize(scip, requiredsize);
   assert(newsize > *arraysize);
   SCIP_CALL( SCIPreallocBlockMemoryArray(scip, array, *arraysize, newsize) );
   *arraysize = newsize;

   return SCIP_OKAY;
}

/** determines all possible covers */
static
SCIP_RETCODE determineCovers(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Longint          capacity,           /**< capacity of knapsack */
   SCIP_Longint*         uniqueweights,      /**< unique weights of variables in knapsack constraint */
   int                   nuniqueweights,     /**< number of unique weights in knapsack constraint */
   int*                  vargroupbegins,     /**< array indicating where a new weight group of variables begins */
   int**                 covers,             /**< pointer to array of store covers */
   int*                  ncovers,            /**< pointer to counter for the number of covers found */
   int                   maxncovers,         /**< maximum number of covers per knapsack */
   int*                  lencovers,          /**< pointer to the maximal number of elements that can be stored in
                                              *   covers */
   SCIP_Longint**        coverweights,       /**< pointer to array to store weight of covers */
   int*                  lencoverweights,    /**< pointer to the maximal number of elements that can be stored in
                                              *   coverweights */
   SCIP_Bool             allowcomplcovermethod /**< whether use the compl cover method */
   )
{
   SCIP_Bool coversterminated = FALSE;
   SCIP_Bool success;
   int* currentcover;
   int* upperbounds;
   SCIP_Longint knapsackweight = 0;
   SCIP_Bool invert = FALSE;
   int i;
   SCIP_Longint coverweight = 0;
   SCIP_Longint complcoverweight = 0;
   int newsize;

   assert(scip != NULL);
   assert(capacity >= 0);
   assert(uniqueweights != NULL);
   assert(nuniqueweights >= 1);
   assert(vargroupbegins != NULL);
   assert(covers != NULL);
   assert(ncovers != NULL);
   assert(maxncovers > 0);
   assert(lencovers != NULL);
   assert(coverweights != NULL);
   assert(lencoverweights != NULL);

   /* initialize data */
   *ncovers = 0;
   *lencovers = nuniqueweights;
   *lencoverweights = 0;
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, covers, *lencovers) );
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, coverweights, *lencoverweights) );

   SCIP_CALL( SCIPallocClearBufferArray(scip, &currentcover, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &upperbounds, nuniqueweights) );
   for( i = 0; i < nuniqueweights; ++i )
      upperbounds[i] = vargroupbegins[i + 1] - vargroupbegins[i];

   for( i = 0; i < nuniqueweights; ++i )
      knapsackweight += uniqueweights[i] * upperbounds[i];

   /* if the total weight of the knapsack is very close to the capacity, it is faster to use the complement method */
   assert(knapsackweight > capacity);
   if( allowcomplcovermethod && knapsackweight  < 2 * capacity )
      invert = TRUE;

   if( invert )
   {
      /* if we use the complement method, the seed should start full */
      for( i = 0; i < nuniqueweights; ++i )
         currentcover[i] = upperbounds[i];
      coverweight = knapsackweight;
   }

   while( !coversterminated && *ncovers < maxncovers )
   {
      /* based on the previous cover, determine seed for starting the search for the next cover (in rev-lex order) */
      SCIP_CALL( findNextCoverSeed(scip, currentcover, nuniqueweights, uniqueweights, upperbounds, &coverweight,
            capacity, &coversterminated, invert, knapsackweight) );

      /* if we found new cover, add it to the list of covers */
      if( !coversterminated )
      {
         /* make sure that we can store the cover */
         if( (*ncovers + 1) * nuniqueweights > *lencovers )
            SCIP_CALL( increaseArraysize(scip, covers, lencovers, (*ncovers + 1) * nuniqueweights) );

         /* make sure we can store the weight of the cover */
         if( (*ncovers + 1) > *lencoverweights )
         {
            newsize = SCIPcalcMemGrowSize(scip, *ncovers + 1);
            assert(newsize > *ncovers);
            SCIP_CALL( SCIPreallocBlockMemoryArray(scip, coverweights, *lencoverweights, newsize) );
            *lencoverweights = newsize;
         }

         /* store the cover and its weight */
         for( i = 0; i < nuniqueweights; ++i )
         {
            assert((*ncovers) * nuniqueweights + i < *lencovers);
            (*covers)[(*ncovers * nuniqueweights) + i] = currentcover[i];
         }
         (*coverweights)[*ncovers] = coverweight;
         *ncovers += 1;

         /* increment the coverseed for the next iteration */
         if( invert )
         {
            /* if inverted, the incrementation must be also done in the other direction */
            for( i = 0; i < nuniqueweights; ++i )
               currentcover[i] = upperbounds[i] - currentcover[i];
            complcoverweight = knapsackweight - coverweight;

            SCIP_CALL( getNextRevLex(scip, currentcover, 0, nuniqueweights, upperbounds, uniqueweights, &complcoverweight,
                  &success) );

            for( i = 0; i < nuniqueweights; ++i )
               currentcover[i] = upperbounds[i] - currentcover[i];
            coverweight = knapsackweight - complcoverweight;
         }
         else
         {
            SCIP_CALL( getNextRevLex(scip, currentcover, 0, nuniqueweights, upperbounds, uniqueweights, &coverweight,
                  &success) );
         }

         if( !success )
            coversterminated = TRUE;
      }
   }

   SCIPfreeBufferArray(scip, &upperbounds);
   SCIPfreeBufferArray(scip, &currentcover);

   return SCIP_OKAY;
}

/** determine the lifting coefs
 *
 *  Loops over all the covers given to find the corresponding pi coeficients and all possible induced independent sets
 *  for the lifted cover inequality.
 */
static
SCIP_RETCODE determineLifts(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Longint          capacity,           /**< capacity of the knapsack */
   SCIP_Longint*         uniqueweights,      /**< array of the different weights, sorted */
   int                   nuniqueweights,     /**< number of different weights */
   int*                  vargroupbegins,     /**< array indicating where a new weight group of variables begins */
   int**                 covers,             /**< pointer to array of all the minimal covers */
   int                   ncovers,            /**< number of minimal covers */
   int**                 picoefs,            /**< pointer to array of lifting coefficient outside of cover */
   int*                  picoefssize,        /**< pointer to the allocated length for picoefs */
   int                   maxnindepsets,      /**< maximum number of independent sets per cover */
   int**                 indepsets,          /**< pointer to array for storing independent sets for each cover */
   int*                  indepsetssize,      /**< pointer to the allocated length of indepset */
   int**                 indepsetgroupbegins,/**< pointer to array storing where a new group of indep sets begins
                                              *   (per cover) */
   int                   coefupperbound      /**< maximum value for the picoefs */
   )
{
   int j;
   int c;
   ...
   SCIP_Longint* mu;
   int* curindepset;
   SCIP_Longint curdelta;
   SCIP_Bool juststarted;
   SCIP_Bool outofoptions;
   SCIP_Bool success;
   int indepsetcounter = 0;
   SCIP_Real* curslopes;
   int* curperm;
   int* tmpset; /* dummy array initialized outside of function for optimization */
   SCIP_Bool coeftoobig;
   SCIP_Bool earlystop = FALSE;
   ...
   assert(vargroupbegins != NULL);
   assert(covers != NULL);
   assert(ncovers >= 0); /* possible to have 0 covers. Then it just stores empty arrays. */
   assert(picoefs != NULL);
   assert(picoefssize != NULL);
   assert(maxnindepsets >= 0);
   assert(indepsets != NULL);
   assert(indepsetssize != NULL);
   assert(indepsetgroupbegins != NULL);

   /* initialize data */
   *indepsetssize = ncovers * nuniqueweights;
   *picoefssize = ncovers * nuniqueweights;
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, picoefs, *picoefssize) );
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, indepsets, *indepsetssize) );
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, indepsetgroupbegins, ncovers + 1) );

   SCIP_CALL( SCIPallocBufferArray(scip, &upperbounds, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &curcover, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &ubslack, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &curpi, nuniqueweights) );
   /* the mu function needs to have as many elements as there can be in any minimal cover.
    * The worst case is then the capacity divided by the smallest weight, +1 because it is a cover.
    * Additionaly, we will also access mu[nitemsincover] thus we need the second +1.
    */
   SCIP_CALL( SCIPallocBufferArray(scip, &mu, (int)capacity / uniqueweights[0] + 2) );
   SCIP_CALL( SCIPallocBufferArray(scip, &curindepset, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &curperm, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &curslopes, nuniqueweights) );
   SCIP_CALL( SCIPallocBufferArray(scip, &tmpset, nuniqueweights) );

   for( j = 0; j < nuniqueweights; ++j)
      upperbounds[j] = vargroupbegins[j + 1] - vargroupbegins[j];

   /* loop over all covers */
   for( c = 0; c < ncovers; ++c )
   {
      nitemsincover = 0;
      coeftoobig = FALSE;

      /* store temporarily the values of the cover */
      idx = c * nuniqueweights;
      for( j = 0; j < nuniqueweights; ++j )
      {
         curcover[j] = (*covers)[idx];
         ubslack[j] = upperbounds[j] - (*covers)[idx];
         nitemsincover += (*covers)[idx++];
      }
      assert(nitemsincover <= (int)((int)capacity / uniqueweights[0] + 1));

      /* get lifting coeficients and mu */
      SCIP_CALL( determinePiCoefs(scip, curcover, uniqueweights, nuniqueweights, curpi, nitemsincover, mu) );

      /* store lifting coefficients */
      idx = c * nuniqueweights;
      for( j = 0; j < nuniqueweights; ++j )
      {
         assert(idx < *picoefssize);

         /* check if they are not too big */
         if( curpi[j] > coefupperbound )
         {
            coeftoobig = TRUE;
            curpi[j] = coefupperbound;
         }

         (*picoefs)[idx++] = curpi[j];
      }

#ifdef PRINT_COVER_INFO
      SCIPinfoMessage(scip, NULL, "found cover:");
      for( j = 0; j < nuniqueweights; ++j )
      {
         SCIPinfoMessage(scip, NULL, " %d (%lld)", curcover[j], uniqueweights[j]);
      }
      SCIPinfoMessage(scip, NULL, " <= %lld \n\tlifting coefs:", capacity);
      for( j = 0; j < nuniqueweights; ++j )
      {
         SCIPinfoMessage(scip, NULL, " %d (%lld)", curpi[j], uniqueweights[j]);
      }
      SCIPinfoMessage(scip, NULL, "\n");
#endif

      /* prepare for maximal independent sets search */
      success = TRUE;
      juststarted = TRUE;
      outofoptions = FALSE;
      earlystop = FALSE;
      curdelta = mu[nitemsincover] - capacity;

      /* if one of the coefficients was artificially adjusted, cancel indepset search */
      if( coeftoobig )
         earlystop = TRUE;

      /* if @p delta equals 1 then there cannot be non-trivial independent sets */
      if( curdelta == 1 )
         earlystop = TRUE;

      /* set indep set to all zeros by default */
      for( j = 0; j < nuniqueweights; ++j )
         curindepset[j] = 0;

      SCIP_CALL( getSlopes(scip, uniqueweights, nuniqueweights, curpi, curslopes) );
      SCIP_CALL( getPermForSlopes(scip, nuniqueweights, curperm, curpi, curslopes) );

      (*indepsetgroupbegins)[c] = indepsetcounter;

      /*if we do not want to look at independent sets, add the zero array and pretend there are none left */
      if( maxnindepsets == 0 || earlystop )
      {
         idx = indepsetcounter * nuniqueweights;

         /* check that we can store the zero indepset */
         if( idx + nuniqueweights > *indepsetssize )
            SCIP_CALL( increaseArraysize(scip, indepsets, indepsetssize, idx + nuniqueweights) );

         for( j = 0; j < nuniqueweights; ++j )
            (*indepsets)[idx++] = curindepset[j];
         ++indepsetcounter;
         outofoptions = TRUE;
      }

      /* Main loop over the possible independent sets */
      while( !outofoptions )
      {
         if( juststarted )
            SCIP_CALL( getMaxLexIndepset(scip, curindepset, 0, nuniqueweights, ubslack, uniqueweights,
                  curpi, mu, nitemsincover, curdelta, curperm) );
         else
            SCIP_CALL( findNextMaxIndepset(scip, curindepset, tmpset, nuniqueweights, uniqueweights, ubslack,
                  curpi, mu, nitemsincover, curdelta, curperm, &success) );
         if( !success )
            outofoptions = TRUE;

         /* if we found an independent set */
         if( !outofoptions || juststarted )
         {
            /* check that we have enough allocated space */
            if( (indepsetcounter + 1) * nuniqueweights > *indepsetssize )
               SCIP_CALL( increaseArraysize(scip, indepsets, indepsetssize, (indepsetcounter + 1) * nuniqueweights) );
            ...
            if( indepsetcounter - (*indepsetgroupbegins)[c] >= maxnindepsets )
               outofoptions = TRUE;

#ifdef PRINT_COVER_INFO
            SCIPinfoMessage(scip, NULL, "\tindep. set:");
            for( j = 0; j < nuniqueweights; ++j )
            {
               SCIPinfoMessage(scip, NULL, " %d", curindepset[j]);
            }
            SCIPinfoMessage(scip, NULL, "\n");
#endif
         }
   ...
   (*indepsetgroupbegins)[ncovers] = indepsetcounter;

   SCIPfreeBufferArray(scip, &tmpset);
   SCIPfreeBufferArray(scip, &curslopes);
   SCIPfreeBufferArray(scip, &curperm);
   SCIPfreeBufferArray(scip, &curindepset);
   SCIPfreeBufferArray(scip, &mu);
   SCIPfreeBufferArray(scip, &curpi);
   SCIPfreeBufferArray(scip, &ubslack);
   SCIPfreeBufferArray(scip, &curcover);
   SCIPfreeBufferArray(scip, &upperbounds);
   return SCIP_OKAY;
}

/* tries to create sparse knapsack data */
static
SCIP_RETCODE tryCreateSparseKnapsackData(
   SCIP*                 scip,               /**< SCIP pointer */
   SCIP_CONS*            cons,               /**< constraint for which data shall be created */
   int                   minsparsity,        /**< minimum allowed sparsity */
   int                   maxsparsity,        /**< maximum allowed sparsity */
   int                   maxncovers,         /**< maximum number of covers per knapsack */
   int                   maxnindepsets,      /**< maximum number of independent sets per cover */
   int                   coefupperbound,     /**< maximum value for the picoefs */
   SCIP_Bool             allowcomplcovermethod, /**< whether use compl cover method when suited */
   SCIP_Bool             handlesmallunitweights /**< whether sparse data shall also be used
                                                 *   for small knapsacks with unit weights */
   )
{
   SCIP_CONSDATA* consdata;
   SCIP_SPARSEDATA* sparsedata;
   SCIP_Longint* sparseweights;
   SCIP_Longint* weights;
   SCIP_Longint knapsackweight = 0;
   SCIP_VAR** sortedvars;
   SCIP_Bool notcreatedata;
   int* vargroupbegins;
   int* nitems;
   int cumnitems;
   int nvars;
   ...
   consdata->checkedsparseknapsack = TRUE;

   SCIP_CALL( SCIPallocBufferArray(scip, &sparseweights, maxsparsity) );
   SCIP_CALL( SCIPallocBufferArray(scip, &nitems, maxsparsity) );

   /* get non-negative weights and sort them */
   nvars = consdata->nvars;
   SCIP_CALL( SCIPallocBufferArray(scip, &sortedvars, nvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, &weights, nvars) );
   for( v = 0; v < nvars; ++v )
   {
      weights[v] = consdata->weights[v];
      sortedvars[v] = consdata->vars[v];
   }

   SCIPsortLongPtr(weights, (void**)sortedvars, nvars);

   SCIP_CALL( SCIPallocBufferArray(scip, &vargroupbegins, maxsparsity + 1) );

   /* check whether there are not too many different weights */
   cnt = 0;
   cumnitems = 0;
   vargroupbegins[0] = 0;
   for( v = 0; v < nvars - 1; ++v )
   {
      if( SCIPisGT(scip, weights[v + 1], weights[v]) && cnt < maxsparsity )
      {
         nitems[cnt] = v + 1 - cumnitems;
         cumnitems = v + 1;

         assert(SCIPisFeasIntegral(scip, weights[v]));
         sparseweights[cnt++] = weights[v];
         vargroupbegins[cnt] = cumnitems;
      }

      if( cnt == maxsparsity && SCIPisGT(scip, weights[v+1], weights[v]) )
      {
         SCIPfreeBufferArray(scip, &vargroupbegins);
         SCIPfreeBufferArray(scip, &weights);
         SCIPfreeBufferArray(scip, &sortedvars);
         SCIPfreeBufferArray(scip, &nitems);
         SCIPfreeBufferArray(scip, &sparseweights);

         return SCIP_OKAY;
      }
   }

   /* do not handle knapsacks with too small sparsity if knapsacks are sufficiently large */
   notcreatedata = cnt < minsparsity - 1;
   if( handlesmallunitweights && nvars <= SMALL_KNAPSACK && cnt == 0 )
      notcreatedata = FALSE;

   if( cnt < maxsparsity )
   {
      sparseweights[cnt] = weights[v];
      nitems[cnt++] = nvars - cumnitems;
      vargroupbegins[cnt] = nvars;
   }

   for( v = 0; v < cnt; ++v )
      knapsackweight += sparseweights[v] * (vargroupbegins[v + 1] - vargroupbegins[v]);
   if( knapsackweight <= consdata->capacity )
      notcreatedata = TRUE;
   ...
   /* store sparse data */
   consdata->issparseknapsack = TRUE;
   SCIP_CALL( SCIPallocBlockMemory(scip, &consdata->sparsedata) );

   sparsedata = consdata->sparsedata;
   sparsedata->nsepacuts = 0;
   sparsedata->disabledsepa = FALSE;
   sparsedata->capacity = consdata->capacity;
   sparsedata->nuniqueweights = cnt;
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, &sparsedata->uniqueweights, sparsedata->nuniqueweights) );
   for( v = 0; v < sparsedata->nuniqueweights; ++v )
      sparsedata->uniqueweights[v] = sparseweights[v];
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, &sparsedata->vars, nvars) );
   for( v = 0; v < nvars; ++v )
      sparsedata->vars[v] = sortedvars[v];
   sparsedata->nvars = nvars;
   SCIP_CALL( SCIPallocBlockMemoryArray(scip, &sparsedata->vargroupbegins, sparsedata->nuniqueweights + 1) );
   for( v = 0; v < sparsedata->nuniqueweights + 1; ++v )
      sparsedata->vargroupbegins[v] = vargroupbegins[v];

   SCIP_CALL( determineCovers(scip, sparsedata->capacity, sparsedata->uniqueweights, sparsedata->nuniqueweights,
         sparsedata->vargroupbegins, &sparsedata->covers, &sparsedata->ncovers, maxncovers, &sparsedata->lencovers,
         &sparsedata->coverweights, &sparsedata->lencoverweights, allowcomplcovermethod) );

   SCIP_CALL( determineLifts(scip, sparsedata->capacity, sparsedata->uniqueweights, sparsedata->nuniqueweights,
         sparsedata->vargroupbegins, &sparsedata->covers, sparsedata->ncovers, &sparsedata->picoefs,
         &sparsedata->picoefssize, maxnindepsets, &sparsedata->indepsets,
         &sparsedata->indepsetssize, &sparsedata->indepsetgroupbegins, coefupperbound) );
...
/** given an LCI, checks if the solution violates it or not  */
static
SCIP_RETCODE separateOneLCI(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Real*            vals,               /**< array of the values for a solution, sorted per weights */
   SCIP_CONS*            cons,               /**< constraint of the knapsack */
   SCIP_VAR**            vars,               /**< variables in knapsack constraint */
   int*                  varmap,             /**< array containing for every value the corresponding variable's index */
   int*                  invvarmap,          /**< inverse of varmap */
   int                   nvars,              /**< number of elements in vals */
   int                   nuniqueweights,     /**< number of unique weights in the knapsack */
   int*                  vargroupbegins,     /**< array for the number of elements in each weight class */
   int*                  cover,              /**< array indicating how many elements per weight class are in the cover*/
   int*                  picoefs,            /**< array corresponding to the pi coefficients */
   int*                  indepset,           /**< array indicating how many of the pi coefficients can get +1 */
   int                   cutid,              /**< ID used for separated cut */
   SCIP_Bool*            infeasible,         /**< pointer to store whether problem is shown to be infeasible */
   SCIP_Bool*            separated,          /**< pointer to store whether cut has been added */
   SCIP_GUBSET*          gubset              /**< partitioning of variables into GUBs */
   )
{
   SCIP_Bool* addedtogub;
   int i;
   int w;
   int nitemsincover = 0;
   SCIP_Real partialsum = 0.0;
   ...
   assert(vars != NULL);
   assert(varmap != NULL);
   assert(gubset == NULL || invvarmap != NULL);
   assert(nvars > 0);
   assert(nuniqueweights > 0);
   assert(vargroupbegins != NULL);
   assert(cover != NULL);
   ...
   *infeasible = FALSE;
   ...
   if( gubset != NULL )
   {
      SCIP_CALL( SCIPallocClearBufferArray(scip, &addedtogub, nvars) );
   }

   /* determine the number of items in the cover */
   for( w = 0; w < nuniqueweights; ++w )
      nitemsincover += cover[w];

   /* iterate over the weight classes */
   for( w = 0; w < nuniqueweights; ++w )
   {
      /* since pi coeffs are positive integer, they are either greater than one or 0 */
      if( picoefs[w] > 0.5 )
      {
         for( i = vargroupbegins[w]; i < vargroupbegins[w + 1]; ++i )
         {
            partialsum += vals[i];
            if( i >= cover[w] && picoefs[w] > 1.5 )
               partialsum += vals[i] * (SCIP_Real)(picoefs[w] - 1);
            if( i >= vargroupbegins[w + 1] - indepset[w] )
               partialsum += vals[i];
         }
      }
      else
      {
         ...
         /* if the pi coefs are 0, then only the elements in the cover and from the indep set count, with coef 1 */
         for( i = vargroupbegins[w + 1] - 1; i >= vargroupbegins[w + 1] - cover[w] - indepset[w]; --i )
         {
            partialsum += vals[i];

            if( gubset != NULL )
            {
               /* iterate over all variables in the GUB, if not selected, assign it the coefficient
                * if has the weight of i
                */
               for( j = 0; j < gubset->gubconss[gubset->gubconssidx[varmap[i]]]->ngubvars; ++j )
               {
                  varidx = gubset->gubconss[gubset->gubconssidx[varmap[i]]]->gubvars[j];

                  /* check whether weight of varidx is exactly the weight of i */
                  if( varidx < vargroupbegins[w] || varidx >= vargroupbegins[w + 1] )
                     continue;

                  /* check whether varidx is not contained in the cover and the independent set */
                  invidx = invvarmap[varidx];
                  if( invidx < vargroupbegins[w + 1] - cover[w] - indepset[w] && !addedtogub[invidx] )
                  {
                     partialsum += vals[invidx];
                     addedtogub[invidx] = TRUE;
                  }
               }
            }
         }
   ...
   if( SCIPisEfficacious(scip, partialsum - (SCIP_Real)(nitemsincover - 1)) )
   {
      char name[SCIP_MAXSTRLEN];
      SCIP_ROW* row;
      SCIP_VAR** rowvars;
      ...
      SCIP_CALL( SCIPallocBufferArray(scip, &rowvars, nvars) );
      SCIP_CALL( SCIPallocBufferArray(scip, &rowvals, nvars) );

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_cut_%d", SCIPconsGetName(cons), cutid);

      SCIP_CALL( SCIPcreateEmptyRowCons(scip, &row, cons, name, -SCIPinfinity(scip), nitemsincover - 1,
            SCIPconsIsLocal(cons), FALSE, TRUE) );
      ...
      for( w = 0; w < nuniqueweights; ++w )
      {
         if( picoefs[w] > 0.5 )
         {
            for( i = vargroupbegins[w]; i < vargroupbegins[w + 1]; ++i )
            {
               rowvars[rowsize] = vars[varmap[i]];
               rowvals[rowsize] = 1.0;
               if( i >= cover[w] )
                  rowvals[rowsize] += picoefs[w] - 1;
               if( i >= vargroupbegins[w + 1] - indepset[w] )
                  rowvals[rowsize] += 1.0;
               ++rowsize;
            }
      ...
      assert(rowsize <= nvars);

      SCIP_CALL( SCIPaddVarsToRow(scip, row, rowsize, rowvars, rowvals) );
   ...
   if( gubset != NULL )
   {
      SCIPfreeBufferArray(scip, &addedtogub);
   }

   return SCIP_OKAY;
}

/** separation routine for LCIs */
static
SCIP_RETCODE separateLCI(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONS*            cons,               /**< constraint for which LCIs are separated */
   SCIP_SPARSEDATA*      sparsedata,         /**< data of sparse knapsack for which LCIs are separated */
   SCIP_Real*            vals,               /**< solution to be checked */
   int*                  ncuts,              /**< pointer to store number of separated cuts */
   ...
   )
{
   ...
   *infeasible = FALSE;
   ...
   nuniqueweights = sparsedata->nuniqueweights;
   nvars = sparsedata->nvars;
   ...
   /* sort for each weight class the corresponding elements of the solution */
   for( w = 0; w < sparsedata->nuniqueweights; ++w )
   {
      SCIPsortRealInt(&vals[sparsedata->vargroupbegins[w]], &perm[sparsedata->vargroupbegins[w]],
            sparsedata->vargroupbegins[w + 1] - sparsedata->vargroupbegins[w]);
   }

   /* for GUB separation, we also need the inverse permutation */
   if( gubset != NULL )
   {
   ...
   /* iterate over all minimal covers */
   for( i = 0; i < sparsedata->ncovers && !(*infeasible); ++i )
   {
      /* for each cover, iterate over all LCI for distinct lifting sequence coefficients */
      for( j = 0; j < sparsedata->indepsetgroupbegins[i + 1] - sparsedata->indepsetgroupbegins[i]; ++j )
      {
         /* check if that cut is violated */
         SCIP_CALL( separateOneLCI(scip, vals, cons, sparsedata->vars, perm, invperm, nvars,
               nuniqueweights,
               sparsedata->vargroupbegins,
               &(sparsedata->covers[i * nuniqueweights]),
               &(sparsedata->picoefs[i * nuniqueweights]),
               &(sparsedata->indepsets[(sparsedata->indepsetgroupbegins[i] + j) * nuniqueweights]),
               sparsedata->nsepacuts, infeasible, &separated, gubset) );
...
/* separates sparse knapsack cuts */
static
SCIP_RETCODE separateSparseKnapsackCuts(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONS*            cons,               /**< knapsack constraint */
   SCIP_SPARSEDATA*      sparsedata,         /**< data of sparse knapsack constraint */
   SCIP_SOL*             sol,                /**< primal SCIP solution, NULL for current LP solution */
   SCIP_Bool*            cutoff,             /**< whether a cutoff has been detected */
   int*                  ncuts,              /**< pointer to add up the number of found cuts */
   int*                  nconss,             /**< pointer to add up the number of added constraints */
   int                   coefupperbound,     /**< maximum value of the coefficients for the cut to be considered */
   SCIP_Bool             usegubs             /**< whether GUBs shall be used in cover separation */
   )
{
   SCIP_GUBSET* gubset = NULL;
   SCIP_Real* vals;
   int tmpnsepa;
   ...
   /* check whether separation has been disabled */
   if( sparsedata->disabledsepa )
      return SCIP_OKAY;

   /* avoid trivial cases that should have been handled completely */
   if( sparsedata->nuniqueweights == 2 && sparsedata->uniqueweights[0] == 1
      && sparsedata->uniqueweights[1] == sparsedata->capacity )
   {
      SCIP_CONSDATA* consdata;
   ...
   if( usegubs )
   {
      SCIP_Longint* tmpweights;
      SCIP_Real* solvals;
      int i;
      int g;

      SCIP_CALL( SCIPallocBufferArray(scip, &tmpweights, sparsedata->nvars) );
      for( g = 0; g < sparsedata->nuniqueweights; ++g )
      {
         for( i = sparsedata->vargroupbegins[g]; i < sparsedata->vargroupbegins[g + 1]; ++i )
            tmpweights[i] = sparsedata->uniqueweights[g];
      }
      assert(i == sparsedata->nvars);

      SCIP_CALL( SCIPallocBufferArray(scip, &solvals, sparsedata->nvars) );
      SCIP_CALL( SCIPgetSolVals(scip, sol, sparsedata->nvars, sparsedata->vars, solvals) );

      /* initializes partion of knapsack variables into nonoverlapping GUB constraints */
      SCIP_CALL( GUBsetCreate(scip, &gubset, sparsedata->nvars, tmpweights, sparsedata->capacity) );

      /* constructs sophisticated partition of knapsack variables into nonoverlapping GUBs */
      SCIP_CALL( GUBsetGetCliquePartition(scip, gubset, sparsedata->vars, solvals) );
      assert(gubset->ngubconss <= sparsedata->nvars);

      SCIPfreeBufferArray(scip, &solvals);
      SCIPfreeBufferArray(scip, &tmpweights);
   }

   SCIP_CALL( SCIPallocBufferArray(scip, &vals, sparsedata->nvars) );
   SCIP_CALL( SCIPgetSolVals(scip, sol, sparsedata->nvars, sparsedata->vars, vals) );

   SCIP_CALL( separateLCI(scip, cons, sparsedata, vals, &tmpnsepa, cutoff, gubset) );
...
/** returns whether separation of knapsack cuts is enabled */
static
SCIP_Bool isKnapsackSeparationEnabled(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_Bool             defsepacuts,        /**< SCIP's default separation decision */
   SCIP_CONSHDLRDATA*    conshdlrdata,       /**< data of knapsack conshdlr */
   SCIP_CONSDATA*        consdata,           /**< data of knapsack constraint */
   SCIP_CONS*            cons                /**< constraint for which separation is checked */
   )
{
   assert(conshdlrdata != NULL);
   assert(consdata != NULL);

   /* if SCIP decided to separate cuts, we do so too */
   if( defsepacuts )
      return TRUE;

   /* do not exploit sparse knapsacks in subSCIPs or if disabled */
   if( (!conshdlrdata->usesparseknapsacks) || SCIPgetSubscipDepth(scip) == 0 )
      return FALSE;

   /* ensure that sparsity has been computed */
   if( !consdata->checkedsparseknapsack )
   {
      SCIP_CALL( tryCreateSparseKnapsackData(scip, cons, conshdlrdata->minsparsity, conshdlrdata->sparsity,
            conshdlrdata->maxncovers, conshdlrdata->maxnindepsets, conshdlrdata->coefupperbound,
            conshdlrdata->allowcomplcovermethod, conshdlrdata->handlesmallunitweights) );
   }

   /* we separate cuts for sparse knapsacks */
   if( consdata->issparseknapsack )
      return TRUE;
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   doseparate = isKnapsackSeparationEnabled(scip, sepacuts, conshdlrdata, consdata, cons);
   ...
   {
      /* possibly try to create sparse knapsack data */
      if( !consdata->checkedsparseknapsack && conshdlrdata->usesparseknapsacks )
      {
         SCIP_CALL( tryCreateSparseKnapsackData(scip, cons, conshdlrdata->minsparsity, conshdlrdata->sparsity,
               conshdlrdata->maxncovers, conshdlrdata->maxnindepsets, conshdlrdata->coefupperbound,
               conshdlrdata->allowcomplcovermethod, conshdlrdata->handlesmallunitweights) );
      ...
      if( consdata->issparseknapsack )
      {
         if( conshdlrdata->separatesparsecuts )
         {
            SCIP_CALL( separateSparseKnapsackCuts(scip, cons, consdata->sparsedata, sol,
                  cutoff, ncuts, nconss, conshdlrdata->coefupperbound, conshdlrdata->usesparsegubs) );
         }
         if( conshdlrdata->separateclassiccuts )
         {
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   sepafreq = conshdlrdata->scipsepafreq;
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   {
      SCIP_CALL( separateCons(scip, conshdlrdata, conss[i], NULL,
            sepacardinality, conshdlrdata->usegubs, &cutoff, &ncuts, &naddconss) );
   }
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( i = 0; i < nusefulconss && ncuts < maxsepacuts && !SCIPisStopped(scip); i++ )
   {
      SCIP_CALL( separateCons(scip, conshdlrdata, conss[i], sol,
            sepacardinality, conshdlrdata->usegubs, &cutoff, &ncuts, &naddconss) );
   }
...
#endif
    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/sparsity",
          "maximum number of unique weights such that a knapsack is considered as sparse",
          &conshdlrdata->sparsity, TRUE, DEFAULT_SPARSITY, 1, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/minsparsity",
          "minimum number of unique weights such that a knapsack is considered as sparse",
          &conshdlrdata->minsparsity, TRUE, DEFAULT_MINSPARSITY, 1, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/maxnindepsets",
          "maximum number of independent sets per cover in the knapsack",
          &conshdlrdata->maxnindepsets, TRUE, DEFAULT_MAXNINDEPSETS, 0, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/maxncovers",
          "maximum number of covers per knapsack",
          &conshdlrdata->maxncovers, TRUE, DEFAULT_MAXNCOVERS, 0, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/separateclassiccuts",
          "whether cuts of the classical knapsack conshdlr shall be separated",
          &conshdlrdata->separateclassiccuts, TRUE, DEFAULT_SEPARATECLASSICCUTS, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/separatesparsecuts",
          "whether cuts of the sparse knapsack conshdlr shall be separated",
          &conshdlrdata->separatesparsecuts, TRUE, DEFAULT_SEPARATESPARSECUTS, NULL, NULL) );

    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/coefupperbound",
          "maximum value of the coefficients for the cut to be considered",
          &conshdlrdata->coefupperbound, TRUE, DEFAULT_COEFUPPERBOUND, 0, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/usesparseknapsacks",
          "whether sparse knapsacks shall be used",
          &conshdlrdata->usesparseknapsacks, TRUE, DEFAULT_USESPARSEKNAPSACKS, NULL, NULL) );

    SCIP_CALL( SCIPaddIntParam(scip,
          "constraints/" CONSHDLR_NAME "/scipsepafreq",
          "sepa frequency used to separate classical cuts",
          &conshdlrdata->scipsepafreq, TRUE, DEFAULT_SCIP_SEPAFREQ, -1, INT_MAX, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/usesparsegubs",
          "whether GUBs shall be used for separating sparse covers",
          &conshdlrdata->usesparsegubs, TRUE, DEFAULT_USESPARSEGUBS, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/handlesmallunitweights",
          "whether sparse data shall also be used for small knapsacks with unit weights",
          &conshdlrdata->handlesmallunitweights, TRUE, DEFAULT_HANDLESMALLUNITWEIGHTS, NULL, NULL) );

    SCIP_CALL( SCIPaddBoolParam(scip,
          "constraints/" CONSHDLR_NAME "/allowcomplcovermethod",
          "whether use the complement cover method when suited",
          &conshdlrdata->allowcomplcovermethod, TRUE, ALLOW_COMPLCOVERMETHOD, NULL, NULL) );
