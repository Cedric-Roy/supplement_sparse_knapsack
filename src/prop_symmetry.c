/* COPIED FROM RELEASE OF SCIP 9.0.1 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                           */
...
#define DEFAULT_ORBISACKFORMULATION     0    /**< integer encoding which orbisack formulation shall be used */
#define DEFAULT_ORBISACKMAXROWS        10    /**< max number of rows we want to consider in an orbisack */
#define DEFAULT_EFISINITIAL          TRUE    /**< should LP relaxation of orbisack constraint be in the initial LP? */
#define DEFAULT_ISRELAXONLY         FALSE    /**< is the constraint for the LP relaxation only? */
...
struct SCIP_PropData
{
   ...
   int                   orbisackformulation; /**< integer encoding which orbisack formulation to be used */
   int                   orbisackmaxrows;    /**< max number of rows we want to consider in an orbisack */
   SCIP_Bool             efisinitial;        /**< should LP relaxation of orbisack constraint be in the initial LP? */
   SCIP_Bool             isrelaxonly;        /**< is the constraint for the LP relaxation only? */
...
#define ORBISACK_FORMULATION_CONS         0
#define ORBISACK_FORMULATION_EF_COVER     1
#define ORBISACK_FORMULATION_EF_EXACT1    2
#define ORBISACK_FORMULATION_EF_EXACT2    3
...
/** Cover extended formulation for the orbisack constraints */
static
SCIP_RETCODE addOrbisackEFCover(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_VAR**            involution,         /**< array of the variables, in consecutive pairs linked
                                              *   in the involution */
   int                   nvars,              /**< number of variables */
   char*                 prename,            /**< prefix for the constraint name */
   SCIP_CONS***          linconss,           /**< pointer to the array of linear constraints */
   int*                  nliconss,           /**< number of linear constraints */
   int*                  linconsssize,       /**< size of the array for the constraints */
   int                   orbisackmaxrows,    /**< max number of rows we want to consider  */
   SCIP_Bool             efisinitial,        /**< should the LP relaxation of constraint be in the initial LP? */
   SCIP_Bool             isrelaxonly         /**< is the constraint only for the relaxation */
   )
{
   ...
   assert(nvars % 2 == 0);
   assert(orbisackmaxrows >= 0);

   ncycles = MIN(nvars / 2, orbisackmaxrows);

   /* need an array for the max of the two elements per 2-cycle */
   if( ncycles > 2 )
   {
      SCIP_CALL( SCIPallocBufferArray(scip, &maxvars, ncycles - 2) );
   }
   SCIP_CALL( SCIPallocBufferArray(scip, &tmpvars, ncycles + 2) );
   SCIP_CALL( SCIPallocBufferArray(scip, &tmpvals, ncycles + 2) );

   /* create the variables */
   for( i = 0; i < ncycles - 2; ++i )
   {
      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_maxvar%d", prename, i);

      SCIP_CALL( SCIPcreateVar(scip, &maxvars[i], name, -1.0, 0.0, 0.0, SCIP_VARTYPE_IMPLINT,
            efisinitial, isrelaxonly, NULL, NULL, NULL, NULL, NULL) );
      if( isrelaxonly )
         SCIPvarMarkRelaxationOnly(maxvars[i]);
      SCIP_CALL( SCIPaddVar(scip, maxvars[i]) );
   }
   ...
   /* avoid frequent memory reallocation */
   SCIP_CALL( ensureDynamicConsArrayAllocatedAndSufficientlyLarge(scip, linconss,
         linconsssize, (*nliconss) + MAX(3*ncycles - 4, ncycles)) );
   ...
   {
      tmpvals[0] = -1.0;
      tmpvars[0] = involution[2*(i+1)];
      tmpvars[1] = maxvars[i];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_max_left%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2, tmpvars, tmpvals, -SCIPinfinity(scip),
            0.0, efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );

      assert((*nliconss) < *linconsssize);
      (*linconss)[(*nliconss)++] = cons;

      tmpvals[0] = 1.0;
      tmpvars[0] = involution[2*(i+1) + 1];
      tmpvars[1] = maxvars[i];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_max_right%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2, tmpvars, tmpvals, -SCIPinfinity(scip),
            1.0, efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
   ...
   /* add aggregated cover constraints */
   tmpvars[0] = involution[0];
   tmpvars[1] = involution[1];
   tmpvals[0] = -1.0;
   tmpvals[1] = 1.0;

   (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_cover0", prename);

   SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2, tmpvars, tmpvals, -SCIPinfinity(scip), 0.0,
         efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
   SCIP_CALL( SCIPaddCons(scip, cons) );
   ...
   for( i = 1; i < ncycles; ++i )
   {
      tmpvars[2] = involution[2 * i];
      tmpvars[3] = involution[2 * i + 1];
      if( i > 1 )
      {
         tmpvars[3 + i - 1] = maxvars[i - 2];
         tmpvals[3 + i - 1] = 1.0;
      }

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_cover%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 4 + i - 1, tmpvars, tmpvals, -SCIPinfinity(scip), 0.0,
            efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
   ...
   for( i = 0; i < ncycles - 2; ++i )
   {
      SCIP_CALL( SCIPreleaseVar(scip, &maxvars[i]) );
   }
   ...
   if( ncycles > 2 )
   {
      SCIPfreeBufferArray(scip, &maxvars);
   }

   return SCIP_OKAY;
}

/** implementation of the first extended formulation for orbisack from "Finding Descriptions of Polytopes via Extended
 *  Formulations and Liftings" by Volker Kaibel and Andreas Loos (2011).
 */
static
SCIP_RETCODE addOrbisackEFExact1(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_VAR**            involution,         /**< array of the variables, in consecutive pairs linked
                                              *   in the involution */
   int                   nvars,              /**< number of variables */
   ...
   )
{
   ...
   assert(scip != NULL);
   assert(involution != NULL);
   assert(nvars > 0);
   assert(nvars % 2 == 0);
   assert(linconss != NULL);
   assert(nliconss != NULL);
   assert(linconsssize != NULL);
   assert(orbisackmaxrows >= 0);
   ...
   SCIP_CALL( SCIPallocBufferArray(scip, &tmpvars, ncycles + 2) );
   SCIP_CALL( SCIPallocBufferArray(scip, &tmpvals, ncycles + 2) );
   SCIP_CALL( SCIPallocBufferArray(scip, &y, ncycles) );
   SCIP_CALL( SCIPallocBufferArray(scip, &z, ncycles) );

   /* create the variables */
   for( i = 0; i < ncycles; ++i )
   {
      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_y%d", prename, i);

      SCIP_CALL( SCIPcreateVar(scip, &y[i], name, 0.0, 1.0, 0.0, SCIP_VARTYPE_IMPLINT,
            efisinitial, isrelaxonly, NULL, NULL, NULL, NULL, NULL) );
      if( isrelaxonly )
         SCIPvarMarkRelaxationOnly(y[i]);
      SCIP_CALL( SCIPaddVar(scip, y[i]) );

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_z%d", prename, i);

      SCIP_CALL( SCIPcreateVar(scip, &z[i], name, 0.0, 1.0, 0.0, SCIP_VARTYPE_IMPLINT,
            efisinitial, isrelaxonly, NULL, NULL, NULL, NULL, NULL) );
      if( isrelaxonly )
         SCIPvarMarkRelaxationOnly(z[i]);
      SCIP_CALL( SCIPaddVar(scip, z[i]) );
   }

   /* avoid frequent memory reallocation */
   SCIP_CALL( ensureDynamicConsArrayAllocatedAndSufficientlyLarge(scip, linconss,
         linconsssize, (*nliconss) + 3*ncycles) );

   /*
    * the variables will be stored in the tmpvars array in the form
    * [xi1 zi y1 y2 y3 ... yi] or
    * [xi2 zi y1 y2 y3 ... yi].
    */
   ...
   {
      /* first equation 19 */
      /* xi2 -zi -y1 -y2 ... -y(i-1) <= 0 */
      tmpvars[0] = involution[2 * i + 1];
      tmpvars[1] = z[i];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_19_%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2 + i, tmpvars, tmpvals, -SCIPinfinity(scip),
            0.0, efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
      ...
      /* then equation 18 */
      /* xi1 -zi -y1 -y2 ... -y(i-1) -yi <= 0 */
      tmpvars[0] = involution[2 * i];
      tmpvars[2 + i] = y[i];
      tmpvals[2 + i] = -1.0;

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_18_%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2 + i + 1, tmpvars, tmpvals, -SCIPinfinity(scip),
            0.0, efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
      ...
      /* we have every thing in place to add the negation of equation 20 */
      /* ___ -zi -y1 -y2 ... -y(i-1) -yi >= -1 */
      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_20_%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, 2 + i, &tmpvars[1], &tmpvals[1], -1.0,
            SCIPinfinity(scip), efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
...
/** implementation of the second extended formulation for orbisack from "Finding Descriptions of Polytopes via Extended
 *  Formulations and Liftings" by Volker Kaibel and Andreas Loos (2011).
 */
static
SCIP_RETCODE addOrbisackEFExact2(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_VAR**            involution,         /**< array of the variables, in consecutive pairs linked
                                              *   in the involution */
   int                   nvars,              /**< number of variables */
   ...
   )
{
   ...
   /* avoid frequent memory reallocation */
   SCIP_CALL( ensureDynamicConsArrayAllocatedAndSufficientlyLarge(scip, linconss,
         linconsssize, (*nliconss) + 4*ncycles + 1) );
   ...
   for( i = 0; i < ncycles; ++i )
   {
      ...
      /* yi -xi1 <= 0 */
      tmpvals[1] = -1.0;
      tmpvars[1] = involution[2 * i];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_28_%d", prename, i);
      ...
      /* yi +xi2 <= 1 */
      tmpvals[1] = 1.0;
      tmpvars[1] = involution[2 * i + 1];

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_29_%d", prename, i);
   ...
   for( i = 0; i < ncycles; ++i )
   {
      ...
      /* first equation 30 */
      /* -xi1 +xi2 -y1 -y2 ... -y(i-1) +yi <= 0 */
      tmpvals[0] = -1.0;
      tmpvals[1] = 1.0;

      tmpvals[2 + i] = 1.0;

      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_30_%d", prename, i);

      SCIP_CALL( SCIPcreateConsLinear(scip, &cons, name, i + 3, tmpvars, tmpvals, -SCIPinfinity(scip),
            0.0, efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons) );
      ...
      (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_27_%d", prename, i);
   ...
   /* equation 31 */
   /* Note that it is of the form y1 +y2 ... +yi <= 1. This means that for each growing i,
    * the new inequality dominates the previous. In other words, we can just add the last one
    */
   (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "%s_eq_31", prename);

   SCIP_CALL( SCIPcreateConsSetpack(scip, &cons, name, ncycles, y,
         efisinitial, TRUE, !isrelaxonly, !isrelaxonly, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
   SCIP_CALL( SCIPaddCons(scip, cons) );
...
/** handles symresack constraints */
static
SCIP_RETCODE handleSymresackCons(
   SCIP*                 scip,               /**< SCIP data structure */
   int*                  perm,               /**< permutation array */
   SCIP_VAR**            permvars,           /**< array of the variables */
   int                   npermvars,          /**< number of variables in the array */
   char*                 name,               /**< name of the constraint */
   SCIP_CONS***          orbconss,           /**< orbisack constraint array */
   SCIP_CONS***          linconss,           /**< linear constraint array */
   int*                  norbconss,          /**< pointer to the number of orb constraints */
   int*                  nlinconss,          /**< pointer to the number of lin constraints */
   int*                  orbconsssize,       /**< pointer to the allocated size of the orb array */
   int*                  linconsssize,       /**< pointer to the allocated size of the lin array */
   int*                  nsymresacks,        /**< pointer to the number of symresack */
   SCIP_Bool             conssaddlp,         /**< should the symmetry breaking constraints be added to the LP? */
   int                   orbisackmethod,     /**< indicate which method is used */
   int                   orbisackmaxrows,    /**< max number of rows we want to consider */
   SCIP_Bool             efisinitial,        /**< should the LP relaxation of constraint be in the initial LP? */
   SCIP_Bool             isrelaxonly         /**< is the constraint only for the relaxation */
   )
{
   ...
   SCIP_VAR*** orbisackvars;
   SCIP_VAR** involution;
   SCIP_Bool isinvolution = TRUE;
   int nbininvvars = 0;
   int nrows;
   int v;
   ...
   assert(npermvars > 0);
   assert(orbconss != NULL);
   assert(linconss != NULL);
   assert(norbconss != NULL);
   assert(nlinconss != NULL);
   assert(orbconsssize != NULL);
   assert(linconsssize != NULL);
   assert(nsymresacks != NULL);
   assert(orbisackmaxrows >= 0);

   if( orbisackmethod == ORBISACK_FORMULATION_CONS )
   {
      SCIP_CONS* cons;
      SCIP_CALL( SCIPcreateSymbreakCons(scip, &cons, name, perm, permvars, npermvars, FALSE,
            conssaddlp, TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons));

      /* do not release constraint here - will be done later */
      SCIP_CALL( ensureDynamicConsArrayAllocatedAndSufficientlyLarge(scip, orbconss,
            orbconsssize, (*norbconss) + 1) );
      (*orbconss)[(*norbconss)++] = cons;
      ++(*nsymresacks);

      return SCIP_OKAY;
   }

   SCIP_CALL( SCIPallocBufferArray(scip, &involution, npermvars) );

   /* detect if we have an involution */
   for( v = 0; v < npermvars; ++v )
   {
      if( !SCIPvarIsBinary(permvars[v]) )
         continue;
      ...
      /* only handle each cycle once */
      if( perm[v] <= v )
         continue;

      assert(nbininvvars < npermvars - 1);
      involution[nbininvvars++] = permvars[v];
      involution[nbininvvars++] = permvars[perm[v]];
   }
   ...
   if( !isinvolution )
   {
      ...
      SCIPfreeBufferArray(scip, &involution);

      /* add the corresponding symresack constraint */
      SCIP_CALL( SCIPcreateSymbreakCons(scip, &cons, name, perm, permvars, npermvars, FALSE,
            conssaddlp, TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
      SCIP_CALL( SCIPaddCons(scip, cons));
   ...
   /* for orbisacks of packing/partitioning type, we do not add extended formulations */
   assert(nbininvvars % 2 == 0);
   nrows = nbininvvars / 2;
   SCIP_CALL( SCIPallocBufferArray(scip, &orbisackvars, nrows) );
   for( v = 0; v < nrows; ++v )
   {
      SCIP_CALL( SCIPallocBufferArray(scip, &orbisackvars[v], 2) );
      orbisackvars[v][0] = involution[2*v];
      orbisackvars[v][1] = involution[2*v + 1];
   }

   SCIP_CALL( SCIPisPackingPartitioningOrbitope(scip, orbisackvars, nrows, 2, NULL, NULL, &type) );
   if( type == SCIP_ORBITOPETYPE_FULL )
   {
      if( orbisackmethod == ORBISACK_FORMULATION_EF_COVER )
      {
         SCIP_CALL( addOrbisackEFCover(scip, involution, nbininvvars, name,
               linconss, nlinconss, linconsssize, orbisackmaxrows, efisinitial, isrelaxonly) );
      }
      else if ( orbisackmethod == ORBISACK_FORMULATION_EF_EXACT1 )
      {
         SCIP_CALL( addOrbisackEFExact1(scip, involution, nbininvvars, name,
               linconss, nlinconss, linconsssize, orbisackmaxrows, efisinitial, isrelaxonly) );
      }
      else
      {
         assert(orbisackmethod == ORBISACK_FORMULATION_EF_EXACT2);
         SCIP_CALL( addOrbisackEFExact2(scip, involution, nbininvvars, name,
               linconss, nlinconss, linconsssize, orbisackmaxrows, efisinitial, isrelaxonly) );
      }
   }
   else
   {
      ...
      for( v = 0; v < nrows; ++v )
         vars1[v] = involution[2*v];
      SCIP_CALL( SCIPallocBufferArray(scip, &vars2, nrows) );
      for( v = 0; v < nrows; ++v )
         vars2[v] = involution[2*v + 1];

      SCIP_CALL( SCIPcreateConsOrbisack(scip, &cons, name, vars1, vars2, nrows, TRUE,
            type == SCIP_ORBITOPETYPE_PARTITIONING, FALSE, conssaddlp, TRUE, FALSE, TRUE, TRUE,
   ...
   for( v = nrows - 1; v >= 0; --v )
   {
      SCIPfreeBufferArray(scip, &orbisackvars[v]);
   }
   SCIPfreeBufferArray(scip, &orbisackvars);
   ...
   )
{
   ...
   if ( norbitopesincomp == 1 )
   {
      ...
      for (k = 0; k < npermsincomp; ++k)
      {
         ...
         SCIP_CALL( handleSymresackCons(scip, propdata->perms[propdata->components[propdata->componentbegins[cidx] + k]],
               propdata->permvars, propdata->npermvars, name, &propdata->genorbconss, &propdata->genlinconss,
               &propdata->ngenorbconss, &propdata->ngenlinconss, &propdata->genorbconsssize, &propdata->genlinconsssize,
               &propdata->nsymresacks, propdata->conssaddlp, propdata->orbisackformulation, propdata->orbisackmaxrows, propdata->efisinitial, propdata->isrelaxonly) );
   ...
   if ( nvarslexorder > 0 && propdata->addsymresacks && ! handlednonbinarysymmetry )
   {
      ...
      for (k = 0; k < npermsincomp; ++k)
      {
         ...
         SCIP_CALL( handleSymresackCons(scip, symresackperm, modifiedpermvars, propdata->npermvars, name,
               &propdata->genorbconss, &propdata->genlinconss, &propdata->ngenorbconss, &propdata->ngenlinconss,
               &propdata->genorbconsssize, &propdata->genlinconsssize, &propdata->nsymresacks, propdata->conssaddlp,
               propdata->orbisackformulation, propdata->orbisackmaxrows, propdata->efisinitial, propdata->isrelaxonly) );
...
{ /*lint --e{641}*/
   ...
   for (p = componentbegins[cidx]; p < componentbegins[cidx + 1]; ++p)
   {
      ...
      if ( propdata->nleaders > 0 && ISSSTBINACTIVE(propdata->sstleadervartype) )
      {
         ...
         SCIP_CALL( handleSymresackCons(scip, modifiedperms[permidx], modifiedpermvars, npermvars, name,
               &propdata->genorbconss, &propdata->genlinconss, &propdata->ngenorbconss, &propdata->ngenlinconss,
               &propdata->genorbconsssize, &propdata->genlinconsssize, &propdata->nsymresacks, propdata->conssaddlp,
               propdata->orbisackformulation, propdata->orbisackmaxrows, propdata->efisinitial, propdata->isrelaxonly) );
      }
      ...
      {
         SCIP_CALL( handleSymresackCons(scip, perms[permidx], permvars, npermvars, name,
               &propdata->genorbconss, &propdata->genlinconss, &propdata->ngenorbconss, &propdata->ngenlinconss,
               &propdata->genorbconsssize, &propdata->genlinconsssize, &propdata->nsymresacks, propdata->conssaddlp,
               propdata->orbisackformulation, propdata->orbisackmaxrows, propdata->efisinitial, propdata->isrelaxonly) );
      }
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/orbisackformulation",
         "which orbisack formulation shall be used (0 = constraint, 1 = EF covers, 2 = EF exact 1, 3 = EF exact 2)",
         &propdata->orbisackformulation, TRUE, DEFAULT_ORBISACKFORMULATION, 0, 3, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "propagating/" PROP_NAME "/orbisackmaxrows",
         "max number of rows we want to consider in an orbisack",
         &propdata->orbisackmaxrows, TRUE, DEFAULT_ORBISACKMAXROWS, 0, 2147483647, NULL, NULL) );
   SCIP_CALL( SCIPaddBoolParam(scip,
         "propagating/" PROP_NAME "/efisinitial",
         "should the LP relaxation of constraint be in the initial LP?",
         &propdata->efisinitial, TRUE, DEFAULT_EFISINITIAL, NULL, NULL) );
   SCIP_CALL( SCIPaddBoolParam(scip,
         "propagating/" PROP_NAME "/isrelaxonly",
         "is the constraint for the LP relaxation only?",
         &propdata->isrelaxonly, TRUE, DEFAULT_ISRELAXONLY, NULL, NULL) );
